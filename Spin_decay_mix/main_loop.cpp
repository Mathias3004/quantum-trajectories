...
int main ( int argc, char *argv[] )
{
    ...
    //  model parameters
    //auto Omega = input.getReal("Omega",1.0);
    auto delta = input.getReal("delta",0.0);
    auto gamma = input.getReal("gamma",1.0);
    //auto p = input.getReal("p",0.0);
    ...
    //outputfiles
    auto output = input.getString("output","data") + "_" + to_string(N_site) + "_" + to_string(Omega) + "_" + to_string(p);
    ...
    if (env.rank() == 0){
      cout << "\n\n*****************************************************************\n";
      cout << "\nRunning system with Omega/gamma=";
      cout << Omega/gamma;
      cout << " and p=" << p;
    ...
    auto H = H_drive(sites,delta,Omega,gamma);
    auto U = toExpH<ITensor>(H,Cplx_i*dt);
    
    auto C_gates = Decay(sites,gamma,p);
    ...
    for (int it = 0; it < N_loop; it++){
      ...
      // make  class instance
      TGateQT<ITensor> t_qt(dt, U, C_gates, psi_init, args, env, true);
